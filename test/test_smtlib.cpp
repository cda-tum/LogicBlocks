...
TEST(TestSMTLib, TestPrint) {
    ...
    LogicTerm const a = smtLibLogic->makeVariable("a", CType::BOOL);
    LogicTerm const b = smtLibLogic->makeVariable("b", CType::INT);
    LogicTerm const c = smtLibLogic->makeVariable("c", CType::REAL);
    LogicTerm const d = smtLibLogic->makeVariable("d", CType::BITVECTOR, 8);

    LogicTerm const t = a && (b == LogicTerm(1));
    t.print(std::cout);
    t.prettyPrint(std::cout);

    LogicTerm const f = (c == LogicTerm(1.0)) && (d == LogicTerm(1, 8));
    f.print(std::cout);
...
TEST(TestSMTLib, TestBasicBooleanAnd) {
    ...
    std::string const output = "; Generated by LogicBlock\n"
    ...
    LogicTerm const p = smtLibLogic->makeVariable("p", CType::BOOL);
    LogicTerm const t = p && !p;
    smtLibLogic->assertFormula(t);
...
TEST(TestSMTLib, TestBasicBooleanImpl) {
    ...
    const std::string output = "; Generated by LogicBlock\n"
                               "(set-option :produce-models true)\n"
                               "(set-logic QF_UF)\n"
                               "(declare-const p_0 Bool)\n"
                               "(assert (=> p_0 (not p_0)))\n"
