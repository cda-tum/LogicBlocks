...
template <typename T> inline T &asAny(Value &);
template <typename T> inline const T &asAny(const Value &);

template <> inline const std::string &asAny<std::string>(const Value &value) {
  return value.data.string;
}

template <> inline std::string &asAny<std::string>(Value &value) {
  return value.data.string;
}

template <typename T> inline std::string fromAny(T &);
template <typename T> inline std::string fromAny(const T &);
template <typename T> inline void toAny(T &, const std::string &);

#define JSON_STD_FROM(T)                                                       \
  template <> inline std::string fromAny<T>(T & value) {                       \
    return std::to_string(value);                                              \
  }                                                                            \
  template <> inline std::string fromAny<T>(const T &value) {                  \
    return std::to_string(value);                                              \
  }

#define JSON_FROM(T, toString)                                                 \
  template <> inline std::string JSON::fromAny<T>(T & value) {                 \
    return toString(value);                                                    \
  }                                                                            \
  template <> inline std::string JSON::fromAny<T>(const T &value) {            \
    return toString(value);                                                    \
  }
...
template <> inline void toAny<int>(int &value, const std::string &str) {
  value = std::stoi(str);
}
...
template <typename T> inline Value toJson(const T &object) {
  Value data;
