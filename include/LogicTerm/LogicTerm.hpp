...
private:
  ...
  explicit LogicTerm(const std::shared_ptr<TermImpl> &impl) : pImpl(impl) {}
  // explicit LogicTerm(OpType opType) : pImpl(makeLogicTerm(termType, opType))
  // {}

public:
  LogicTerm(bool value) : pImpl(makeLogicTerm(value)) {}
  LogicTerm(int value) : pImpl(makeLogicTerm(value)) {}
  LogicTerm(double value) : pImpl(makeLogicTerm(value)) {}
